...
//		obstacles[2].ypoints[3];
	...
	public DriverAction update(boolean wasResetAfterCollision) {
		...
		richtung = lenkung(x, y, 1000, 1000);
		//System.out.println(harmReihe());
		
		
		
		return new DriverAction(0.5f, richtung);
	}
...
//		System.out.println("Wink: " + wink);
		...
		float zielWinkel = atan2Vector(bx,by);
		System.out.println("zielWinkel: " + zielWinkel);
		
		float endWinkel = zielWinkel - ori;
		System.out.println("endWinkel: " + endWinkel);

		if (ori > endWinkel + 0.2) { //0.785pi = ca 45°
			richtung = -1;
			return richtung;
		} else if(ori < endWinkel - 0.2){
			richtung = 1;
			return richtung;
		}else {
			richtung = 0;
...
//	}
	...
	public float atan2Vector(double x, double y) {
		float zielWinkel = (float) Math.atan2(y, x);
